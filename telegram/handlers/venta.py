    ...
    def prepare_venta(chat_id, user, args, original_text, resolver_almacen_fn, buscar_presentacion_fn):
        ...
        for item in items_raw:
            ...
            if stock_actual < cantidad:
                warning_msg = f"⚠️ Stock insuficiente en {almacen_nombre} para '{presentacion.nombre}'. Solicitado: {cantidad}, Disponible: {stock_actual}."
                # Buscar alternativas
                alt_invs = Inventario.query.filter(Inventario.presentacion_id == presentacion.id, Inventario.almacen_id != almacen_id, Inventario.cantidad > 0).all()
                if alt_invs:
                    alt_stocks = {}
                    for inv in alt_invs:
                        alt_stocks[inv.almacen.nombre] = alt_stocks.get(inv.almacen.nombre, 0) + float(inv.cantidad)
                    alt_hints = []
                    for w_name, w_stock in alt_stocks.items():
                        if w_stock >= float(cantidad):
                            alt_hints.append(f"{w_name} ({w_stock})")
                    if alt_hints:
                        warning_msg += f"💡 Hay stock en: {', '.join(alt_hints)}. Si deseas vender desde ahí, menciona el almacén en tu mensaje."
    ...
    def prepare_ventas_lote(chat_id, user, args, original_text, resolver_almacen_fn, buscar_presentacion_fn):
        ...
        for v in ventas_raw:
            ...
            for item in items_raw:
                ...
                stock_disp = sum(float(inv.cantidad) for inv in invs)
                if stock_disp < float(cantidad):
                    warning_msg = f"⚠️ Stock bajo en {almacen_nombre} para '{presentacion.nombre}'. Req: {cantidad}, Disp: {stock_disp}."
                    # Buscar alternativas
                    alt_invs = Inventario.query.filter(Inventario.presentacion_id == presentacion.id, Inventario.almacen_id != almacen_id, Inventario.cantidad > 0).all()
                    if alt_invs:
                        alt_stocks = {}
                        ...
                        for w_name, w_stock in alt_stocks.items():
                            if w_stock >= float(cantidad):
                                alt_hints.append(f"{w_name} ({w_stock})")
                        if alt_hints:
                            warning_msg += f"\n   💡 Hay stock en: {', '.join(alt_hints)}. Menciona el almacén en tu mensaje para usarlo."
                    warnings.append(warning_msg)
